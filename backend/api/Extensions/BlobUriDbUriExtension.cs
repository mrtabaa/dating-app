...
public static class BlobUriDbUriExtension
{
    /// <summary>
    ...
    public static string? ConvertBlobUriToDbUri(string? blobUri)
    {
        if (blobUri is null) return null;

        string? imagePath = blobUri?.Substring(blobUri.IndexOf("photos/") + "photos/".Length);
